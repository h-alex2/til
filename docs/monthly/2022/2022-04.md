...
- 첫회사에서 처음 맡은 일
- 회사에서 신입에게 기대하는 정도
- 취업전으로 돌아간다면 어떻게 공부하고 싶으신지 궁금합니다
- 좋은 회사, 좋지 않은 회사를 어떻게 구분하시는지 궁금합니다.
- 동기분들 중에 관두신 분들이 혹시 꽤 계신지..
- 개발자가 되고 나서 제일 좋은 점과 힘든 점
- JS 기초를 탄탄히 하고나면 다른 것들 공부할 때 조금은 더 쉽게 배울 수 있을까
- 이론적인 공부를 더 많이 하는 게 좋을지 프로젝트 위주로 하는 것이 좋을지
- 취업 전에 뭘 제일 중점적으로 공부하는 게 좋을지
- 알고리즘 또는 자료구조 책 또는 개발관련 책 추천 해주실 것이 있나용? 
- 영어능력
- 회사에서 맥을 쓰는 경우가 많을까요? 


- 이거 문제 물어볼까..!!?
## 1. `hashTableHelpers.js`에 `limitedArray.storage` 추가
...
```js
HashTable.prototype.resize = function (loadFactor) {
  ...
  const moveData = (value) => {
    console.log("1", oldStorage.storage(), value)
    if (!value) {
      return null;
    } else {
      let k, v;
    ...
    console.log("2", oldStorage.storage(), value)
  };
  oldStorage.each(moveData);
};
```
